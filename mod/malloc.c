...
INTERPOSE(void *, malloc, size_t n)
{
    ...
    int err = capture_before(EVENT_MALLOC, &ev);
...
INTERPOSE(void *, calloc, size_t n, size_t s)
{
    ...
    int err = capture_before(EVENT_CALLOC, &ev);
...
INTERPOSE(void *, realloc, void *p, size_t n)
{
    ...
    int err = capture_before(EVENT_REALLOC, &ev);
...
INTERPOSE(void, free, void *p)
{
    ...
    int err = capture_before(EVENT_FREE, &ev);
    REAL(free, p);
    if (err != PS_DROP)
...
INTERPOSE(int, posix_memalign, void **memptr, size_t alignment, size_t size)
{
    ...
    int err = capture_before(EVENT_POSIX_MEMALIGN, &ev);
...
INTERPOSE(void *, aligned_alloc, size_t alignment, size_t size)
{
    ...
    int err = capture_before(EVENT_ALIGNED_ALLOC, &ev);
